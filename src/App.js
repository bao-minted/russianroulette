...
fire() {
    ...
    let playerChoice= this.state.chambers[Math.floor(Math.random()*this.state.chambers.length)];
    let computerChoice=this.state.chambers[Math.floor(Math.random()*this.state.chambers.length)];
    let message='ALIVE'
    let idx
    let count = this.state.count
    console.log('gg',this.state.players.length)
    ...
    if (playerChoice===computerChoice){
      let clone = this.state.players.filter(name => name !== this.state.currentU)
      message = 'DEAD'
      this.setState({
        chambers:[],
        count: count+1,
  ...
  render() {
    ...
    return (
      <div className="App">
       ...
       <h2>Chance of dying: {parseInt(1*100/this.state.chambers.length) > -1 ? parseInt(1*100/this.state.chambers.length) :  100}%</h2> 
       ...
       <div>
         ...
         <p> Player: { this.state.players.length>0 ? this.state.currentU:''}</p>
       {!this.state.isPause ? <button onClick={()=> this.fire()}>Pull the trigger</button> : <></>}
       <button onClick={()=> this.setState({chambers:[0,1,2,3,4,5], message:'', count:0})}>Replay</button>
       </div>}
